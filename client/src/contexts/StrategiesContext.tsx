...
export function StrategiesProvider({ children }: StrategiesProviderProps) {
  ...
  const [todayStrategy, setTodayStrategyState] = useState<{ strategyId: number; date: string } | null>(null);
  ...
  const getStrategy = useCallback((id: number): Strategy | undefined => {
  ...
  const toggleFavorite = useCallback(async (strategyId: number) => {
    if (!currentUser) {
  ...
  const isFavorite = useCallback((strategyId: number): boolean => {
  ...
  // Set daily strategy
  const setDailyStrategy = useCallback(async (strategyId: number) => {
    if (!currentUser) {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : "Failed to set daily strategy";
      setError(errorMessage);
      throw new Error(errorMessage);
    }
  }, [currentUser]);
