...
export async function updateUserSettings(userId: string, settings: Partial<Omit<UserSettings, "userId" | "updatedAt">>): Promise<void> {
  try {
    const docRef = doc(db, "users", userId, "settings", "preferences");
    await setDoc(docRef, {
      ...settings,
...
export async function getTodaysStrategy(userId: string): Promise<{ strategyId: number; date: string } | null> {
  try {
